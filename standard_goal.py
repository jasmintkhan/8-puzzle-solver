...
def dfs(start_state, goal_state):
    ...
    stack = [(start_state, [])]  # each element in stack is (current_state, path_to_this_state)
    ...
    while stack:
        ...
        # print the current state being explored in 3x3 grid format
...
def bfs(start_state, goal_state):
    ...
    queue = deque([(start_state, [])])  # each element is (current_state, path_to_this_state)
    visited = set()  # avoid revisiting
    node_expansions = 0  # track the number of node expansions
...
def ucs(start_state, goal_state):
    ...
    while pq:
        ...
        if current_state not in visited:
            ...
            for move, new_state in get_neighbors(current_state):
                if new_state not in visited:
                    # each move's cost = 1, so add 1 to the cumulative cost
                    heapq.heappush(pq, (cumulative_cost + 1, new_state, path + [move]))
    
    return None, node_expansions  # if no solution is found

# calculate manhattan distance
def manhattan_distance(state, goal_state="_12345678"):
    distance = 0
...
def euclidean_distance(state, goal_state="_12345678"):
    distance = 0
    state_list = list(state)
    for i, tile in enumerate(state_list):
        if tile != "_":
